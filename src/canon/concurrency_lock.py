...
def write_new_lock(lock_dir: Path, lock_name: str, token: str) -> Path:
    ...
    if _can_use_dir_fd_lock():
        ...
        return lock_path
    _write_token_to_new_lock(lock_path, token, _LOCK_FLAGS)
    try:
        _verify_created_lock(lock_dir, snapshot, lock_path, token)
    except LockError:
        _cleanup_created_lock(lock_path, token)
        raise
    return lock_path
...
def _write_token_to_new_lock(target: str | Path, token: str, flags: int, *, dir_fd: int | None = None) -> None:
    ...
    try:
        ...
        _close_fd(fd)
        if dir_fd is None:
            _cleanup_partial_lock(Path(target))
        else:
...
def _cleanup_created_lock(path: Path, token: str) -> None:
    try:
        if stat.S_ISREG(path.lstat().st_mode) and path.read_text(encoding="utf-8") == token:
            path.unlink()
