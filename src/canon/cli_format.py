...
def write_result(
    ...
    *,
    ...
    if json_output:
        stdout.write(_json_result_text(result))
...
def _validate_result(result: CliResult) -> None:
    ...
    if type(result.exit_code) is not int or isinstance(result.exit_code, bool):
        raise TypeError("invalid CLI result exit_code")
...
def _snapshot_data(data: Mapping[str, object] | None) -> dict[str, object] | None:
    if data is None:
        return None
...
def _snapshot_mapping(data: Mapping[object, object]) -> dict[str, object]:
    try:
        return {_snapshot_key(key): _snapshot_value(value) for key, value in data.items()}
    except Exception:
